"""Frozen paired local-agent comparison: unified public evidence vs summary.

The v2 business rules, tool environment, sampling and summary reducer are
unchanged. V4 uses the SAME text-plan then task-bound JSON actor in BOTH arms.
Only the candidate compiles current observations together with retained facts.
...
from rcwt_agent_run import (ACTION_MAX_TOKENS, ROOT, actor_messages as base_actor_messages, append, dump,
...
from rcwt_decision_v4 import task_schema, ordered_messages
from rcwt_review_v3 import planning_messages, review_messages

SCHEMA = "rcwt-online-memory/4"
ACTOR_INSTRUCTION_VARIANT = "ordered_public_rule_walkthrough_v1"
REVISION_PLAN_SHA256 = "f56c44d3cf62f99b8fd01d00d731d240f9372ff0aaf846390282acaf7c03582d"
POLICIES = ("summary", "structured")
DATASET_SEEDS = {"development": 2026091205, "confirmatory": 2026091206}
INFERENCE_SEED = 20260911
SCHEDULE_SEED = 2026091208
...
SOURCE_NAMES = (
    ...
    "rcwt_review_v3.py",
    "rcwt_context_v4.py", "rcwt_decision_v4.py", "rcwt_online_v4.py", "rcwt_analysis_v4.py",
)
...
def actor_messages(memory: str, public_step: dict) -> list[dict]:
    """Same constant public-rule clarification in both arms; no action selection."""
    return ordered_messages(base_actor_messages(memory, public_step))
...
def perform_step(client, simulator, memory, policy, index, budget) -> dict:
    ...
    # The stored text remains the only persistent state. The new transient
    # view overlays current PUBLIC tool records before literal joins. It never
    # uses the oracle or recommends a decision, and is NOT writer state.
    actor_memory = (build_context(memory, public, tokenize=client.tokenize, budget=budget)
    ...
    draft = client.complete(planning_messages(messages), max_tokens=ACTION_MAX_TOKENS,
                            schema=None, purpose=f"draft:{policy}")
    ...
    call = client.complete(review_messages(messages, draft.text), max_tokens=ACTION_MAX_TOKENS,
                           schema=task_schema(public), purpose=f"action:{policy}")
    ...
    return {"public_step": public, "memory_before": memory, "actor_memory_before": actor_memory,
            ...
            "draft_format": "unconstrained_text_plan", "draft_action": None,
            "draft_evidence_check": None,
            "action": action,
...
def prepare(directory: Path, args) -> dict:
    ...
    if file_hash(ROOT / "docs/rcwt_online_v4_revision_02.md") != REVISION_PLAN_SHA256:
        raise ValueError("The registered last-development revision plan changed")
    ...
    count = 8 if args.mode == "development" else 32
    ...
    historical_v3 = read(ROOT / "results/agent_v3_development/attempt_05/protocol.json")["source_sha256"]
    if any(sources().get(path) != digest for path, digest in historical_v3.items()):
        raise ValueError("All eleven v3 sources must remain unchanged")
    ...
    protocol = {"schema": SCHEMA, "frozen_at_utc": utc_now(), "mode": args.mode,
                ...
                "schedule_seed": args.schedule_seed, "analysis_seed": 2026091207,
                ...
                "actor_instruction_variant": ACTOR_INSTRUCTION_VARIANT,
                ...
                "candidate_origin": "New user-requested series after the negative v3 closure; engineered public evidence compilation, not autonomous RSI",
                "memory_intervention": "Original structured post-action storage plus a transient context that overlays current public tool records onto retained state and joins the requested case before projection; may change the first-step input",
                "baseline": "Original summary reducer, public rules, simulator and sampler; shared text-plan then task-bound JSON actor with a constant ordered-public-rule walkthrough in both arms. The schema binds case and operation, never factual eligibility or a chosen action",
                ...
                "v3_protocol_sha256": file_hash(ROOT / "results/agent_v3_development/attempt_05/protocol.json"),
                "v3_index_sha256": file_hash(ROOT / "results/agent_v3_development/development.json"),
                "v3_closure_sha256": file_hash(ROOT / "results/agent_v3_development/RESULTS.md")}
...
def validate_protocol(directory: Path) -> dict:
    ...
    expected_mode = {"development": ("train", 8), "confirmatory": ("test", 32)}
    if (protocol.get("schema") != SCHEMA or protocol.get("mode") not in expected_mode
            ...
            or protocol.get("draft_format") != "unconstrained_text_plan"
            or protocol.get("actor_instruction_variant") != ACTOR_INSTRUCTION_VARIANT
            or type(protocol.get("development_revision")) is not int or protocol["development_revision"] != 2
            or protocol.get("revision_plan_sha256") != REVISION_PLAN_SHA256
            or protocol.get("action_schema") != "Public task identity and operation only, identical in both arms; no factual eligibility constraints or semantic veto"
            ...
            or type(protocol["analysis_seed"]) is not int or protocol["analysis_seed"] != 2026091207
    ...
    historical_v3 = read(ROOT / "results/agent_v3_development/attempt_05/protocol.json")["source_sha256"]
    if any(protocol["source_sha256"].get(path) != digest for path, digest in historical_v3.items()):
        raise ValueError("Inherited v3 sources changed")
    for label, name in (("v3_protocol_sha256", "attempt_05/protocol.json"),
                        ("v3_index_sha256", "development.json"), ("v3_closure_sha256", "RESULTS.md")):
        if protocol[label] != file_hash(ROOT / "results/agent_v3_development" / name):
            raise ValueError("Historical v3 evidence changed")
...
def report(directory: Path) -> dict:
    ...
    if protocol["mode"] == "development":
        ...
        result = analyze_v4(rows, expected_count=32, bootstrap_samples=10000, seed=2026091207)
        dump(directory / "analysis.json", result)
        (directory / "RESULTS.md").write_text(render_report_v4(result, protocol, verification),
...
def main():
    ...
    parser.add_argument("--dataset-seed", type=int, default=2026091205)
    parser.add_argument("--inference-seed", type=int, default=20260911)
    parser.add_argument("--schedule-seed", type=int, default=2026091208)
