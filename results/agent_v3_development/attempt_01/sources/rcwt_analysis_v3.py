"""Offline, paired confirmation for an engineered memory-only v3 candidate.
    ...
    *,
    ...
    return {
        ...
        "evaluation_cost": {
            ...
            "scope": "both supplied test arms; includes memory compaction, excludes earlier development, setup and unmeasured inter-episode overhead",
        },
        "limitations": [
            "The structured candidate is an engineered deterministic memory compactor, not an autonomously learned policy, model-weight update, or evidence of recursive self-improvement.",
            "The intended intervention is memory-only against the unchanged summary baseline; the independent frozen-run verifier must establish the actual implementation and input binding.",
            ...
            "Tokens and step/episode latency include memory compaction; decision latency is shown separately. Local latency also depends on hardware, load, cache state, and execution order.",
...
def render_report_v3(analysis: dict[str, Any], protocol: dict[str, Any], verification: dict[str, Any]) -> str:
    ...
    lines = [
        "# RCWT-S Online v3: paired memory-only confirmation",
        "",
        f"Result: **{status}**.",
        "",
        "This compares an engineered deterministic structured-memory compactor with the unchanged LLM summary baseline. The structured compactor makes no LLM memory calls; agent decisions still use the local model. This is not autonomous policy learning or a production-deployment certificate.",
        "",
    ...
    lines += [
        "", "Step latency includes memory compaction and other measured per-step work; decision latency is separate. Episode wall time includes all steps. No timing uncertainty or universal speed claim is inferred from these descriptive measurements.",
    ...
    lines += [
        "", f"Both test arms: {total['model_calls']} model calls; {total['total_tokens']} tokens; {total['accounted_episode_wall_seconds']:.3f} s accounted episode wall time. Tokens, calls, and wall time include memory compaction; earlier development, setup, and unmeasured inter-episode overhead are excluded.",
