    ...
    *,
    ...
    return {
        ...
        "evaluation_cost": {
            ...
            "scope": "both supplied test arms; includes the draft and mandatory LLM review, memory compaction and query-conditioned retrieval, excludes earlier development, setup and unmeasured inter-episode overhead",
        },
        "limitations": [
            ...
            "This compares combined writing-and-reading against the unchanged summary memory policy, conditional on the same frozen two-pass actor in both arms. Draft plus mandatory LLM self-review is a shared actor-workflow change, not the unchanged end-to-end v2 actor workflow. The comparison does not isolate a self-review effect.",
            ...
            "Both arms always generate a draft and one reviewed final action, each with a 512-token output cap. Only the final action is executed. Neither deterministic rule correction, oracle feedback, draft fallback, nor a selective retry based on correctness is part of the actor workflow.",
            ...
            "Stored memory and the query-conditioned actor view are separately capped at 256 actual model tokens; the actor receives the view, not both texts concatenated. Empty stored memory yields an empty view, preserving identical first-step draft inputs. Each final review additionally receives its own raw draft. The writer consumes stored memory, not its selected view.",
...
def render_report_v3(analysis: dict[str, Any], protocol: dict[str, Any], verification: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"Result: **{status}**.",
        "",
        "This compares engineered deterministic memory storage plus query-conditioned retrieval with the unchanged LLM summary memory policy, conditional on the same frozen two-pass actor in both arms. Each actor always drafts and then performs one LLM self-review; only the final action is executed. The structured writer and reader make no LLM memory calls. This is not autonomous policy learning or a production-deployment certificate.",
        "",
        "Draft plus mandatory review is a shared actor-workflow change, not the unchanged end-to-end v2 actor workflow. Both arms use the same base actor prompt, review instruction, local model and 512-token cap per pass. No deterministic rule correction, oracle feedback, fallback to the draft or correctness-triggered retry is applied. The comparison does not isolate the effect of self-review.",
        "",
        ...
        "",
        "Stored state and the actor's selected memory view each have a 256-token cap. The actor receives the selected view, not the two texts together; the current-step input stays unchanged. Empty stored state gives an empty view, keeping the first-step draft inputs identical. Final review additionally sees its own raw draft. The writer still receives the stored state.",
        "",
