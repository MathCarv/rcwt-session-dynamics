...
from rcwt_local_model import LocalModelClient, SAMPLING, canonical_hash

ROOT = Path(__file__).resolve().parents[1]
SCHEMA = "rcwt-online-agent/2"
ACTION_MAX_TOKENS = 512
...
def run_episode(client, episode, policy: Policy, budget: int, trace_path: Path) -> dict:
    ...
    for index in range(len(episode.steps)):
        ...
        action_call = client.complete(actor_messages(memory_before, public), max_tokens=ACTION_MAX_TOKENS,
                                      schema=ACTOR_SCHEMA, purpose=f"action:{policy.name}")
        ...
        action = extract_action(action_call.text, action_call.finish_reason)
        evidence_check = extract_evidence_check(action_call.text, action_call.finish_reason)
...
def prepare(directory: Path, args) -> dict:
    ...
    protocol = {"schema": SCHEMA, "created_at_utc": utc_now(), "seed": args.seed, "counts": counts,
                ...
                "actor_schema_sha256": canonical_hash(ACTOR_SCHEMA),
                ...
                "inference": {"local_only": True, "api_cost_usd": 0, "seed": args.seed,
                              "action_sampling": SAMPLING, "memory_sampling": SAMPLING,
                              "max_action_tokens": ACTION_MAX_TOKENS, "action_thinking": False,
                              "memory_thinking": False, "reasoning_budget": 0, "cache_prompt": False},
    ...
    for split_number, split in enumerate(("train", "validation", "test")):
        ...
        for (episode_id, policy), rows in groups.items():
            ...
            for index, row in enumerate(rows):
                ...
                validate_call(action_call, actor_messages(memory, public), ACTION_MAX_TOKENS,
                              ACTOR_SCHEMA, f"action:{policy}")
                raw_action = extract_action(action_call["text"], action_call["finish_reason"])
                if row["action"] != raw_action:
                    raise ValueError("Executed action differs from raw completion or truncation status")
                if row["evidence_check"] != extract_evidence_check(action_call["text"], action_call["finish_reason"]):
                    raise ValueError("Evidence check differs from the actor's uncorrected output")
...
def main() -> None:
    ...
    parser.add_argument("--model", default="rcwt-local-qwen35-4b")
