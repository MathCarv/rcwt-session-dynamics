...
The v2 actor, tool environment, sampling and summary baseline are unchanged.
This is a separate experiment, not a replacement for its negative result.
"""
...
def perform_step(client, simulator, memory, policy, index, budget) -> dict:
    ...
    call = client.complete(actor_messages(memory, public), max_tokens=ACTION_MAX_TOKENS,
    ...
    return {"public_step": public, "memory_before": memory, "action": action,
...
def summarize(records: list[dict], episode_seconds: float) -> dict:
    ...
    return {**{k: records[0][k] for k in ("episode_id", "family", "split", "policy")},
            ...
            "decision_seconds": [r["client_events"][0]["result"]["wall_seconds"] for r in records],
...
def prepare(directory: Path, args) -> dict:
    ...
    protocol = {"schema": SCHEMA, "frozen_at_utc": utc_now(), "mode": args.mode,
                ...
                "candidate_origin": "Engineered memory repair informed by disclosed v2 failures and development; not autonomous RSI",
                "baseline": "Unmodified v2 actor, environment, sampler and LLM rolling-summary policy",
