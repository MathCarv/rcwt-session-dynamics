"""Offline, paired confirmation for an engineered storage-and-retrieval system.
    ...
    *,
    ...
    return {
        ...
        "evaluation_cost": {
            ...
            "scope": "both supplied test arms; includes memory compaction and query-conditioned retrieval, excludes earlier development, setup and unmeasured inter-episode overhead",
        },
        "limitations": [
            ...
            "This is a combined writing-and-reading intervention against the unchanged summary baseline, not compaction alone. Selection, public-record joins, derived-field calculations and stale-field invalidation move work from the LLM to deterministic code; any observed gain belongs to the combined memory system.",
            ...
            "Stored memory and the query-conditioned actor view are separately capped at 256 actual model tokens; the actor receives the view, not both texts concatenated. Empty stored memory yields an empty view, preserving identical first-step actor inputs. The writer consumes stored memory, not its selected view.",
            ...
            "Tokens and step/episode latency include memory compaction and query-conditioned retrieval; local tokenization and read processing are included in step time. Decision latency is shown separately. Local latency also depends on hardware, load, cache state, and execution order.",
...
def render_report_v3(analysis: dict[str, Any], protocol: dict[str, Any], verification: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"Result: **{status}**.",
        "",
        "This compares engineered deterministic memory storage plus query-conditioned retrieval with the unchanged LLM summary baseline. The structured writer and reader make no LLM memory calls; agent decisions still use the unchanged local model and actor prompt. This is not autonomous policy learning or a production-deployment certificate.",
        "",
        "The intervention changes both memory writing and reading, not compaction alone. Deterministic selection, public-record joins, derived-field calculations and stale-field invalidation replace work previously left to the LLM. The reader uses retained facts for the literal requested case, invalidates old fields by same-source/same-ID updates, and does not import current observation values or decide the action. Any gain is evidence about the combined memory system.",
        "",
        "Stored state and the actor's selected memory view each have a 256-token cap. The actor receives the selected view, not the two texts together; the current-step input stays unchanged. Empty stored state gives an empty view, keeping the first-step actor inputs identical. The writer still receives the stored state.",
        "",
    ...
    lines += [
        "", "Step latency includes memory compaction, query-conditioned retrieval, tokenization and other measured per-step work; decision latency is separate. Episode wall time includes all steps. No timing uncertainty or universal speed claim is inferred from these descriptive measurements.",
