"""Offline unified-context comparison under a shared task-bound two-pass actor.
...
SCHEMA_VERSION = "rcwt-online-analysis/4"
...
def analyze_v4(
    ...
    *,
    ...
    for row in rows:
        ...
        # V4 cannot infer a favorable safety result from an absent measurement.
        ...
        if record["split"] != "test":
            raise ValueError("v4 confirmation accepts only test summaries; do not pool development data")
        if record["policy"] not in POLICIES:
            raise ValueError("v4 policies must be exactly summary and structured")
        if record["family"] not in FAMILIES:
            raise ValueError(f"Unknown task family: {record['family']}")
        if record["steps"] != EXPECTED_STEPS:
            raise ValueError("Every v4 episode must contain exactly 8 steps")
    ...
    return {
        ...
        "evaluation_cost": {
            ...
            "scope": "both supplied test arms; includes the free-text plan and mandatory final-JSON pass, memory compaction and unified public-evidence reading, excludes earlier development, setup and unmeasured inter-episode overhead",
        },
        "limitations": [
...
def render_report_v4(analysis: dict[str, Any], protocol: dict[str, Any], verification: dict[str, Any]) -> str:
    ...
    if not isinstance(analysis, dict) or analysis.get("schema_version") != SCHEMA_VERSION:
        raise ValueError("Unsupported v4 analysis schema")
