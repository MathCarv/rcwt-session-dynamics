...
unchanged. Revision 3.3 uses the SAME text-plan then JSON actor in BOTH arms.
...
def prepare(directory: Path, args) -> dict:
    ...
    protocol = {"schema": SCHEMA, "frozen_at_utc": utc_now(), "mode": args.mode,
                ...
                "baseline": "Original v2 public rules, final JSON envelope, environment, sampler and summary reducer, with the same text-planning then final-JSON actor in both arms",
