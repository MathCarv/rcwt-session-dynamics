...
from tools import report_online_v4_development as report
...
class DevelopmentReportTests(unittest.TestCase):
    def setUp(self):
        self.temporary = tempfile.TemporaryDirectory(prefix="rcwt-v4-development-index-test-", dir=report.ROOT)
        ...
        for relative in ("results/agent_v3_development/attempt_05/protocol.json",
                         "results/agent_v3_development/development.json",
                         "results/agent_v3_development/RESULTS.md", "docs/rcwt_online_v4_protocol.md",
                         "docs/rcwt_online_v4_revision_02.md"):
            artifact = self.root / relative
            artifact.parent.mkdir(parents=True, exist_ok=True)
            artifact.write_text("SYNTHETIC historical/registration fixture.\n", encoding="utf-8")
    ...
    def test_series_cap_and_first_passing_candidate_are_enforced(self):
        first = self.fixture("pilot01")
        second = self.fixture("pilot02")
        with self.assertRaisesRegex(ValueError, "first passing"):
            report.build_index([first, second], self.output)
        with self.assertRaisesRegex(ValueError, "one or two"):
            report.build_index([first, second, first], self.output)

    def test_historical_and_registration_provenance_are_bound(self):
        run = self.fixture("pilot01")
        report.generate([run], self.output)
        path = self.root / "docs/rcwt_online_v4_protocol.md"
        path.write_text("ALTERED fixture", encoding="utf-8")
        with self.assertRaisesRegex(ValueError, "JSON"):
            report.verify_index([run], self.output)
