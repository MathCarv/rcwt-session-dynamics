...
def _analyze(rows=None, **kwargs):
    return analyze_v3(_cohort() if rows is None else rows, expected_count=4, bootstrap_samples=100, **kwargs)
...
class AnalysisV3Tests(unittest.TestCase):
    def test_exact_positive_gain_and_measured_resources(self):
        ...
        self.assertEqual(primary["bootstrap_seed"], 2026091203)
    ...
    def test_defaults_are_32_balanced_pairs_and_10000_fixed_seed_bootstraps(self):
        ...
        self.assertEqual(BOOTSTRAP_SEED, 2026091203)
        result = analyze_v3(_cohort(count=32))
        primary = result["comparisons"]["structured_vs_summary"]
    ...
    def test_frozen_helpers_produce_identical_metrics_and_primary_bootstrap(self):
        ...
        self.assertEqual(comparison, _comparison(candidate, baseline, 100, 2026091203))
    ...
    def test_exact_ten_percentage_points_threshold_is_inclusive(self):
        ...
        for index in range(16):
            rows[index * 2 + 1] = _record(index, "structured", 5)
        result = analyze_v3(rows, expected_count=20, bootstrap_samples=1000)
        primary = result["comparisons"]["structured_vs_summary"]
    ...
    def test_parameter_validation(self):
        for expected in (0, -1, True, 4.0, 1, 5, 6, 7):
            with self.subTest(expected=expected), self.assertRaises(ValueError):
                analyze_v3(_cohort(), expected_count=expected)
        for samples in (0, -1, True, 0.5):
            with self.subTest(samples=samples), self.assertRaises(ValueError):
                analyze_v3(_cohort(), expected_count=4, bootstrap_samples=samples)
    ...
    def test_report_exposes_real_scope_resources_and_no_autonomous_learning_claim(self):
        result = _analyze()
        report = render_report_v3(result, {"model": "fake-contract-model", "counts": {"test": 4}},
        ...
        for phrase in (
            ...
            "storage plus query-conditioned retrieval", "not compaction alone", "public-record joins",
            "derived-field calculations", "combined memory system", "does not import current observation values",
            "each have a 256-token cap", "first-step planning inputs identical", "current-step input stays unchanged",
            "same frozen two-pass actor in both arms", "only the final action is executed",
            "not the unchanged end-to-end v2 actor workflow", "does not isolate the effect of planning or self-review",
            "free-text plan", "never parsed into a tool call", "never passed to the memory writer",
            "No deterministic rule correction", "512-token cap per pass",
        ...
        self.assertEqual(report, render_report_v3(result, {"model": "fake-contract-model", "counts": {"test": 4}},
                                                 {"status": "PASS", "verified_steps": 64, "scope": "fake contract receipt, not real inference"}))

    def test_report_does_not_rename_accuracy_with_safety_regression_as_full_pass(self):
        rows = _cohort()
        rows[1]["unsafe_actions"] = 1
        report = render_report_v3(_analyze(rows), {}, {"status": "PASS"})
    ...
    def test_report_does_not_claim_gain_for_tie_or_missing_verification(self):
        report = render_report_v3(_analyze(_cohort(candidate=4)), {}, {"status": "PASS"})
        self.assertIn("**NOT DEMONSTRATED**", report)
        for verification in ({}, {"status": "FAIL"}, {"status": "WARN"}, {"status": True}):
            with self.subTest(verification=verification):
                report = render_report_v3(_analyze(), {}, verification)
    ...
    def test_report_rejects_wrong_schema_or_receipt_types(self):
        ...
        for protocol, verification in ((None, {}), ({}, None), ([], {})):
            with self.subTest(protocol=protocol, verification=verification), self.assertRaises(ValueError):
                render_report_v3(_analyze(), protocol, verification)
