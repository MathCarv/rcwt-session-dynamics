"""Offline v4 integrity tests with an EXPLICITLY FAKE transport.
...
class ExplicitFakeV4Transport(LocalModelClient):
    ...
    def _request(self, path, payload=None):
        ...
        return {"model": self.model,
                "id": "explicit-fake-v4-transport-not-model-evidence",
...
def _fake(protocol=None):
    return ExplicitFakeV4Transport(
...
class OnlineV4Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.network_guard = patch("urllib.request.OpenerDirector.open",
                                  side_effect=AssertionError("Real HTTP forbidden in fake v4 tests"))
        cls.network_guard.start()
        cls.addClassCleanup(cls.network_guard.stop)

        def development_only_generator(split, count, seed):
            if split != "train":
                raise AssertionError("Real confirmatory data generation forbidden in fake v4 tests")
            return generate_episodes(split, count, seed)

        cls.data_guard = patch.object(runner, "generate_episodes", side_effect=development_only_generator)
        cls.data_guard.start()
        cls.addClassCleanup(cls.data_guard.stop)
        cls.temporary = tempfile.TemporaryDirectory(prefix="explicit-fake-rcwt-v4-")
    ...
    def test_fake_development_end_to_end_replays_8_episodes_times_2(self):
        self.assertEqual(self.verification["status"], "PASS")
        self.assertEqual(self.verification["verified_steps"], 128)
        self.assertEqual(self.fake_calls, 312)
        ...
        self.assertEqual(Counter(row["policy"] for row in traces), {"summary": 64, "structured": 64})
        ...
        self.assertEqual(sum(c["prompt_tokens"] for c in calls), 4056)
        self.assertEqual(sum(c["completion_tokens"] for c in calls), 2184)
        self.assertEqual(sum(row["model_calls"] for row in summaries), 312)
        self.assertEqual(sum(row["prompt_tokens"] for row in summaries), 4056)
        self.assertEqual(sum(row["completion_tokens"] for row in summaries), 2184)
        self.assertEqual({policy: sum(row["model_calls"] for row in summaries
                                     if row["policy"] == policy) for policy in runner.POLICIES},
                         {"structured": 128, "summary": 184})
        ...
        for row in traces:
            ...
            self.assertEqual(row["actor_memory_tokens"],
                             len(ExplicitFakeV4Transport.encode_tokens(row["actor_memory_before"])))
            ...
            self.assertEqual(final["arguments"]["schema"], runner.task_schema(row["public_step"]))
    ...
    def test_initial_raw_public_input_and_shared_schema_but_different_evidence_views(self):
        codec = ExplicitFakeV4Transport
        ...
        for episode in episodes:
            ...
            for policy in runner.POLICIES:
                with self.subTest(family=episode.family, policy=policy):
                    ...
                    base = runner.actor_messages(core["actor_memory_before"], episode.public_step(0))
                    self.assertEqual(actor["arguments"]["messages"], runner.planning_messages(base))
                    ...
                    if policy == "summary":
                        ...
                        self.assertTrue(core["actor_memory_before"])
                        self.assertGreater(core["actor_memory_tokens"], 0)
                        self.assertLessEqual(core["actor_memory_tokens"], 256)
                    ...
                    self.assertEqual(final_messages[:2], base)
                    ...
                    self.assertEqual(final["arguments"]["schema"], runner.task_schema(episode.public_step(0)))
            for stage in (0, 1):
                summary, structured = actor_inputs["summary"][stage], actor_inputs["structured"][stage]
                self.assertEqual({k: v for k, v in summary.items() if k != "messages"},
                                 {k: v for k, v in structured.items() if k != "messages"})
                self.assertEqual(summary["messages"][0], structured["messages"][0])
                baseline_input = json.loads(summary["messages"][1]["content"])
                candidate_input = json.loads(structured["messages"][1]["content"])
                self.assertEqual(baseline_input["current_step"], candidate_input["current_step"])
                self.assertEqual(baseline_input["retained_memory"], "")
                self.assertTrue(candidate_input["retained_memory"])
                self.assertNotEqual(summary["messages"], structured["messages"])
        for policy in runner.POLICIES:
            audited = runner.AuditedClient(_fake())
            simulator = Simulator(episodes[0])
            previous = "" if policy == "structured" else "x" * 1025
            with patch.object(runner, "build_context", return_value="x" * 1025):
        ...
        audited = runner.AuditedClient(_fake())
        with patch.object(runner, "build_context", return_value="x" * 1024):
            core = runner.perform_step(audited, Simulator(episodes[0]), "", "structured", 0, 256)
        self.assertEqual(core["actor_memory_tokens"], 256)
        self.assertEqual(core["memory_before"], "")

    def test_context_combines_only_public_updates_and_does_not_replace_writer_state(self):
        episode = generate_episodes("train", 4, runner.DATASET_SEEDS["development"])[0]
        case_id = episode.public_step(0)["task"]["case_id"]
        previous = compact_structured(ExplicitFakeV4Transport.encode_tokens, "", json.dumps({
        ...
        # values, which this new intervention explicitly includes in its card.
        observations = (
        ...
        audited = runner.AuditedClient(_fake())
        reader = runner.build_context
        with patch.object(runner, "build_context", wraps=reader) as read_spy, \
        ...
        self.assertEqual(card["available_evidence"]["payment_status"], "pending")
        self.assertEqual(card["available_evidence"]["invoice_amount_cents"], 9876543)
        self.assertEqual(card["linked_ids"], {"account_id": "NEW_ACCOUNT_VALUE_CANARY",
                         "merchant_id": "NEW_MERCHANT_VALUE_CANARY", "order_id": "NEW_ORDER_VALUE_CANARY"})
        for value in ("cleared", "PRIVATE_", "expected_action", "reason_code"):
    ...
    def test_structured_uses_only_tokenization_and_no_llm_compactor(self):
        ...
        self.assertEqual(len(ExplicitFakeV4Transport.encode_tokens(core["memory_after"])), core["memory_tokens"])
    ...
    def test_task_bound_schema_uses_public_request_after_plan_in_both_arms(self):
        episode = generate_episodes("train", 4, runner.DATASET_SEEDS["development"])[1]
        public = episode.public_step(0)
        original_schema = copy.deepcopy(ACTOR_SCHEMA)
        schema_results = []
        for policy in runner.POLICIES:
            client = _fake()
            original = runner.task_schema

            def observed_schema(public_argument):
                # The common schema is computed only after the plan has returned,
                # from the unchanged raw public step, never the private evaluator.
                self.assertEqual([call.purpose for call in client.calls], ["draft:" + policy])
                self.assertEqual(public_argument, public)
                self.assertNotIn("expected_action", json.dumps(public_argument))
                return original(public_argument)

            audited = runner.AuditedClient(client)
            with patch.object(runner, "task_schema", side_effect=observed_schema) as schema_spy:
                runner.perform_step(audited, Simulator(episode), "", policy, 0, 256)
            schema_spy.assert_called_once_with(public)
            final = _actor_complete(audited.events, "action")
            schema = final["arguments"]["schema"]
            self.assertEqual(final["result"]["request"]["response_format"]["json_schema"]["schema"], schema)
            arguments = schema["properties"]["arguments"]["properties"]
            self.assertEqual(arguments["case_id"]["const"], public["task"]["case_id"])
            self.assertEqual(arguments["decision"]["enum"], ["hold", "ask_info", "approve"])
            self.assertEqual(arguments["amount_cents"],
                             ACTOR_SCHEMA["properties"]["arguments"]["properties"]["amount_cents"])
            self.assertEqual(schema["properties"]["evidence_check"], ACTOR_SCHEMA["properties"]["evidence_check"])
            schema_results.append(schema)
        self.assertEqual(schema_results[0], schema_results[1])
        self.assertEqual(ACTOR_SCHEMA, original_schema)
    ...
    def test_prepare_uses_real_frozen_sources_and_rejects_existing_or_invalid_args(self):
        ...
        self.assertEqual(protocol["schema"], "rcwt-online-memory/4")
        self.assertEqual(len(runner.SOURCE_NAMES), 15)
        self.assertIn("rcwt_retrieval_v3.py", runner.SOURCE_NAMES)
        self.assertIn("rcwt_review_v3.py", runner.SOURCE_NAMES)
        self.assertTrue({"rcwt_context_v4.py", "rcwt_decision_v4.py", "rcwt_online_v4.py", "rcwt_analysis_v4.py"}
                        .issubset(runner.SOURCE_NAMES))
        ...
        self.assertEqual((protocol["count"], protocol["split"], protocol["counts"]), (8, "train", {"train": 8}))
        self.assertEqual(runner.DATASET_SEEDS, {"development": 2026091205, "confirmatory": 2026091206})
        self.assertEqual((runner.SCHEDULE_SEED, protocol["analysis_seed"]), (2026091208, 2026091207))
        self.assertEqual(protocol["source_sha256"], runner.sources())
        history = runner.ROOT / "results/agent_v3_development"
        inherited = runner.read(history / "attempt_05/protocol.json")["source_sha256"]
        self.assertEqual(len(inherited), 11)
        self.assertEqual({key: protocol["source_sha256"][key] for key in inherited}, inherited)
        for label, filename in (("v3_protocol_sha256", "attempt_05/protocol.json"),
                                ("v3_index_sha256", "development.json"), ("v3_closure_sha256", "RESULTS.md")):
            self.assertEqual(protocol[label], runner.file_hash(history / filename))
    ...
    def test_protocol_mutations_are_rejected_even_after_refreezing(self):
        mutations = {
            "schema": "rcwt-online-memory/3.3", "actor_passes": 1, "draft_format": "json_action",
            "action_schema": "Semantic safety veto derived from the hidden oracle",
            "self_review": "Conditional review only after an oracle failure",
            "memory_budget": 255, "count": 4, "counts": {"train": 4}, "split": "test",
            ...
            "v3_protocol_sha256": "0" * 64, "v3_index_sha256": "0" * 64, "v3_closure_sha256": "0" * 64,
        }
    ...
    def test_confirmatory_count_and_seed_contract_without_generating_any_confirmatory_data(self):
        class BlockedConfirmatoryGeneration(Exception):
            pass

        # A deliberately fake passing gate probes only the requested generator
        # arguments. It cannot create a corpus, protocol, selection receipt or
        # model result: the generator is intercepted before all such writes.
        requested, train_generator = [], runner.generate_episodes

        def intercept(split, count, seed):
            if split == "test":
                requested.append((split, count, seed))
                raise BlockedConfirmatoryGeneration
            return train_generator(split, count, seed)

        directory = self.root / (self._testMethodName + "-never-created")
        with patch.object(runner, "development_gate", return_value={"passed": True}), \
                patch.object(runner, "generate_episodes", side_effect=intercept), \
                self.assertRaises(BlockedConfirmatoryGeneration):
            runner.prepare(directory, _args("confirmatory", self.fixture))
        self.assertEqual(requested, [("test", 32, 2026091206)])
        self.assertFalse(directory.exists())
