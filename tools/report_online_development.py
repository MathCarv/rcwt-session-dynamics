"""Index explicitly supplied, trusted v3 development archives without pooling quality.

Usage: python tools/report_online_development.py --runs RUN [RUN ...] --output-dir OUTPUT
...
from tools import verify_online_v3_archive as archive_verifier


SCHEMA = "rcwt-online-development-index/1"
...
def _attempt(directory: Path, output_dir: Path) -> dict:
    ...
    if (not isinstance(protocol, dict) or protocol.get("mode") != "development"
            ...
            or not protocol["schema"].startswith("rcwt-online-memory/")):
...
def build_index(runs: list[Path], output_dir: Path) -> dict:
    if not runs:
        raise ValueError("Supply every development attempt to include")
...
def render_report(index: dict) -> str:
    lines = ["# Online memory: development attempts", "",
