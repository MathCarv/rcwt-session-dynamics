...
from rcwt_review_v3 import planning_messages
        ...
        for policy in POLICIES:
            ...
            for index, (line, row) in enumerate(rows):
                ...
                if draft_calls:
                    ...
                    is_plan = "response_format" not in draft_call.get("request", {})
                    draft = {"text": draft_call["text"], "finish_reason": draft_call["finish_reason"],
                             "format": "unconstrained_text_plan" if is_plan else "json_action_draft",
                             "proposed_action": None if is_plan else extract_action(draft_call["text"], draft_call["finish_reason"]),
                             "evidence_check": None if is_plan else extract_evidence_check(draft_call["text"], draft_call["finish_reason"]),
                    ...
                    if (("draft_action" in row and row["draft_action"] != draft["proposed_action"])
                            ...
                            or ("draft_format" in row and row["draft_format"] != draft["format"])):
                        raise ValueError("Draft metadata differs from the unexecuted raw draft")
                elif "draft_action" in row or "draft_evidence_check" in row or "draft_format" in row:
                ...
                if bound_to_request:
                    ...
                    if draft is not None and "request" in draft_call:
                        ...
                        expected_draft_messages = planning_messages(messages[:2]) if is_plan else messages[:2]
                        if (len(messages) != 4 or canonical_hash(expected_draft_messages) != canonical_hash(draft_call["request"]["messages"])
    ...
    for number, example in enumerate(diagnosis["examples"], 1):
        ...
        for decision_id in example["decision_ids"]:
            ...
            if row["draft"] is not None:
                is_plan = row["draft"]["format"] == "unconstrained_text_plan"
                label = "Free-text plan (NOT EXECUTED; NOT PARSED AS A TOOL CALL):" if is_plan else "Draft model output (NOT EXECUTED):"
                lines += [label, "", "```text",
                          row["draft"]["text"].replace("```", "[backticks escaped]"), "```", "",
                          f"First-pass format: `{row['draft']['format']}`; finish reason: `{row['draft']['finish_reason']}`.", ""]
                if is_plan:
                    lines += ["The raw plan supplies no executable action or extracted evidence check. Its finish reason does not classify the final action as invalid; the original final completion is evaluated independently.", ""]
                else:
                    lines += [f"Draft proposed action, not executed: `{json.dumps(row['draft']['proposed_action'], ensure_ascii=False)}`.", ""]
